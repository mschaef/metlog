...
* Complete
** DONE Plots display with a spinner when there are pending queries
  Implemented this with a grey background.
** DONE Plots display with a gray background when there is stale or missing data
** DONE The minimum and maximum Y values are always displayed.
