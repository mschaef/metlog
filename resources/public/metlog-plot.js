...
import { getSeriesData } from './metlog-data.js';

const EPSILON = 0.00001;

function seconds(s) { return s * 1000; }
function minutes(m) { return seconds(m * 60); }
function hours(h) { return minutes(h * 60); }
function days(d) { return hours(d * 24); }
...
function drawSeries(ctx, w, h, plotBeginT, plotEndT, seriesDefn) {
    const data = getSeriesData(seriesDefn.seriesName);
...
export function canvasSeriesDefn(canvas) {
...
export function setupPlotCanvases() {
    document.querySelectorAll(".series-plot").forEach((canvas) => {
        setupCanvas(canvas);
    });
}

export function updatePlots(beginT, endT) {
    document.querySelectorAll(".series-plot").forEach((canvas) => {
